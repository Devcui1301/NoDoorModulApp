...
public class NoDoorLogEventManager {
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        ...
        NoDoorAdjust.pushTrackEventAdmob(adValue);
        NoDoorAppsflyer.getInstance().pushTrackEventAdmob(adValue, adUnitId, adType);
...
//        logEventWithAds(context, (float) adValue.getRevenue(), 0, adValue.getAdUnitId(), adValue.getNetworkName(), ITGAdConfig.PROVIDER_MAX);
        ...
        NoDoorAdjust.pushTrackEventApplovin(adValue, context);
        NoDoorAppsflyer.getInstance().pushTrackEventApplovin(adValue, adType);
    }
    ...
    private static void logPaidAdImpressionValue(Context context, double value, int precision, String adunitid, String network, int mediationProvider) {
        ...
        NoDoorAdjust.logPaidAdImpressionValue(value, "USD");
    ...
    public static void setEventNamePurchaseAdjust(String eventNamePurchase) {
        NoDoorAdjust.setEventNamePurchase(eventNamePurchase);
    }

    public static void trackAdRevenue(String id) {
        NoDoorAdjust.trackAdRevenue(id);
    }

    public static void onTrackEvent(String eventName) {
        NoDoorAdjust.onTrackEvent(eventName);
    }

    public static void onTrackEvent(String eventName, String id) {
        NoDoorAdjust.onTrackEvent(eventName, id);
    }

    public static void onTrackRevenue(String eventName, float revenue, String currency) {
        NoDoorAdjust.onTrackRevenue(eventName, revenue, currency);
    }

    public static void onTrackRevenuePurchase(float revenue, String currency, String idPurchase, int typeIAP) {
        NoDoorAdjust.onTrackRevenuePurchase(revenue, currency);
        NoDoorAppsflyer.getInstance().onTrackRevenuePurchase(revenue, currency, idPurchase, typeIAP);
    }
    ...
    public static void pushTrackEventApplovin(MaxAd ad, Context context) {
        NoDoorAdjust.pushTrackEventApplovin(ad, context);
    }
}
